...
- Provide --quiet / --verbose toggles via `cli_common.add_verbosity_args` (claude/scripts/dotfiles_cli_common.py, imported as `cli_common`), which also supplies the `qprint` / `vprint` output helpers; gate non-essential stdout with `qprint(..., quiet=quiet)` and diagnostics with `vprint(..., verbose=verbose)`.
