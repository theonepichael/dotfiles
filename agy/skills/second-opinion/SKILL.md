...
`agy` nor `opencode` is on `PATH`, or when the available backend(s) fail (e.g.
the `adversary` agent errors out — check with `opencode run --agent adversary
--auto --format json <text> 2>&1` if that happens). Relay that message and stop
— don't retry or fall back to critiquing the plan yourself.
