...
If the user didn't name a specific topic (and isn't asking for verification),
grill the plan under discussion when the conversation makes it obvious;
otherwise ask the user what to grill before proceeding.
...
2. Ask one question at a time, in plain conversational text — state the question directly, give your recommended answer with brief reasoning, then wait for the user's plain-text response. (Copilot has no structured multi-choice prompt; even when the plausible answers are enumerable, present them as a short list in your message rather than a UI widget.)
...
4. A branch is resolved when the answer generates no new questions. Keep drilling until every open question is decided. Two distinct early exits — the user's words pick which:
   - **Pause** ("let's stop here", "I need to step away", "we'll come back to this") — stop without deciding anything. Open questions stay open for a later resume; no plan is written. Offer a backlog item per the shared instructions file's proactive-capture protocol: session slug in `context`, `next_steps` pointing at `grill.py next`.
...
4. Always offer to clear and go, in plain text: ask "Clear context and start executing this plan now?", recommend yes, and wait for a plain-text answer.
   - **Yes** — run `grill.py mark-pending-execution` (defaults to this session), then tell the user in plain text: "Marked. Start a fresh session whenever you're ready — I'll pick the plan back up automatically." There's no tool to reset the session itself, so the user has to do it.
