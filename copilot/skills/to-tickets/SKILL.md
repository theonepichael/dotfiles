...
4. Show the full slice list with its dependency edges and stop for explicit user confirmation before writing the batch file or invoking the runner — never create tickets silently. Ask in plain text with a recommendation. If the user asks to reorder/merge/split slices, redraft and re-show. Cap this at 3 redraft cycles (matching the `second-opinion` skill's convergence cap); if still unresolved after 3, ask the user to either hand-edit the drafted batch directly or escalate to the `grill-me` skill for alignment on the breakdown, rather than looping further.
5. On confirmation, write the confirmed batch to `~/.claude/data/to-tickets/<topic-slug>-tickets-batch.json` (its own directory — never `~/.claude/data/grill/`, which `grill.py` globs as its private session store and cannot tolerate a non-session file landing in) and run `python3 ~/.claude/scripts/to_tickets_runner.py run <that path>`.
