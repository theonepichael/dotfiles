...
6. Once verification (if any) is settled, always offer clear-and-go, in plain text with a recommendation: "Clear context and start executing this plan now?" On yes: run `grill.py mark-pending-execution` (defaults to this session), then tell the user in plain text: "Marked — start a fresh session whenever you're ready and ask me to pick the plan back up." Pi supports a `session_start` extension event (`docs/extensions.md`), but no extension hooked to it ships in this repo (only the permission-gate and ruff-format extensions are ported — see `pi/CLAUDE_CODE_PARITY.md`), so resume is manual: when a session opens with the user asking to resume/execute the marked plan, run `grill.py pending-plan --consume` and act on the printed instructions (resume if the user says go/continue, otherwise leave the cleared flag alone). On no, nothing else happens, no state change.
