...
## 2. Start
If not already in-progress: `dev_status.py start $ARGUMENTS` (`--if-rev <N>` for numeric ids). On a main/master checkout, `start` now refuses (worktree guard) — do step 3 first, then run `start` from inside the fresh worktree.
